...
Foam::tmp<Foam::volScalarField> preciceAdapter::FSI::ForceBase::mu() const {
  ...
  if (solverType_.compare("incompressible") == 0) {
    if (mesh_.foundObject<fluidThermo>(basicThermo::dictName)) {
      ...
      dimensionedScalar nu(FSIDict.lookup(
          "nu")); // We use this deprecated lookup() for backwards compatibility
    ...
    FatalErrorInFunction << "Did not find the correct mu." << exit(FatalError);
